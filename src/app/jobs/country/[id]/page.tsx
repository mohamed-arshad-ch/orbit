...
export default async function CountryJobsPage({ params }: PageProps) {
    ...
    const countryJobs = jobs.filter((job) =>
        job.location.toLowerCase().includes(country.name.toLowerCase()) ||
        job.location.toLowerCase().includes(id.toLowerCase())
    );
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-6">
                        <div>
                            ...
                            <div className="flex items-center gap-4">
                                ...
                                <h1 className="text-4xl md:text-6xl font-bold text-white">Vacancies in {country.name}</h1>
                            </div>
                            <p className="text-slate-400 mt-4 text-lg">Browse available opportunities in {country.name}.</p>
                        </div>
                        <div className="bg-white/5 border border-white/10 rounded-2xl px-6 py-4 backdrop-blur-sm">
                            <span className="text-sky-400 font-bold text-2xl">{countryJobs.length}</span>
                            <span className="text-slate-400 ml-2">Positions Available</span>
                        </div>
                    </div>
                            ...
                            {countryJobs.map((job) => (
                                <Link href={`/jobs/${job.id}`} key={job.id} className="group">
                                    <div className="h-full p-8 rounded-3xl bg-slate-900/50 border border-white/5 hover:border-sky-500/50 hover:bg-slate-900 transition-all duration-300 backdrop-blur-sm relative overflow-hidden flex flex-col">
                                        ...
                                        <p className="text-slate-400 mb-8">{job.company}</p>
                                        <div className="mt-auto pt-6 border-t border-white/10 flex items-center justify-between">
                                            <div luxury-font="true">
